...
from llava.peft.import_utils import is_bnb_4bit_available, is_bnb_available
from llava.peft.utils import (
    CLAMP_QUANTILE,
...
def get_module_name(module):
    # 自动找到模块的 key，如 "base_model.model.layers.0.self_attn.q_proj"
    for name, m in module.named_modules():
        if m is module:
            return name
    return None

# def init_rando(self, U: Tensor, Vt: Tensor): 
            ...
            if isinstance(target, LoraLayer):
                ...
                if combination_type == "linear":
                    for adapter, weight in zip(adapters, weights):
                        ...
                        # 基础 LoRA 缩放：alpha / r
                        base_scale = target.scaling[adapter]  # 这是原来的 alpha/r

                        # 你们新增的可学习 scale s，如果不存在，就当 1.0
                        learnable_scale = (
                            target.lora_scale[adapter]
                            if hasattr(target, "lora_scale") and adapter in target.lora_scale
                            else 1.0
                        )

                        # 真正的总缩放 = (alpha/r) * s
                        effective_scale = base_scale * learnable_scale
                        target_lora_A.data += current_adapter_lora_A.data * weight * effective_scale
        ...
        for key in key_list:
            ...
            if isinstance(target, LoraLayer):
                for attr in [
                    ...
                    if hasattr(target, attr) and adapter_name in getattr(target, attr):
...
class LoraLayer:
    def __init__(self, in_features: int, out_features: int, **kwargs):
        ...
        # 🔹 每个 adapter 一个可学习缩放参数
        self.lora_scale = nn.ParameterDict({})
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            ...
            # 🔹 新增：可学习缩放参数，初始为 0（刚开始 LoRA 不起作用）
            init_scale = -0.005  # 你想要的初始值

            scale_param = nn.Parameter(
                torch.full((1,), init_scale, dtype=self.weight.dtype, device=self.weight.device),
                requires_grad=True
            )
            self.lora_scale.update(nn.ParameterDict({adapter_name: scale_param}))
            # print("----------------------scale_param--------------------------------")
            # print(f"scale_param={scale_param.detach().view(-1)[0].cpu()}")
            # print("----------------------scale_param--------------------------------")
    ...
    def reset_lora_parameters(self, adapter_name):
        if adapter_name in self.lora_A.keys():
            ...
            # self.init_rando(self.lora_A[adapter_name].weight, self.lora_B[adapter_name].weight)
            # print("----------------------init_rando--------------------------------")
        if adapter_name in self.lora_embedding_A.keys():
            # initialize a the same way as the default for nn.linear and b to zero
            nn.init.zeros_(self.lora_embedding_A[adapter_name])
            nn.init.normal_(self.lora_embedding_B[adapter_name])
            # self.init_rando(self.lora_embedding_A[adapter_name], self.lora_embedding_B[adapter_name])
...
class Linear(nn.Linear, LoraLayer):
    ...
    def get_delta_weight(self, adapter):
        ...
        # 优先用可学习的 lora_scale，没有则回退到原来的 scaling
        if adapter in self.lora_scale:
            return base * self.scaling[adapter] * self.lora_scale[adapter]
        else:
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            lora_out = self.lora_B[self.active_adapter](
                self.lora_A[self.active_adapter](self.lora_dropout[self.active_adapter](x))
            )

            # 优先用可学习的 lora_scale
            if self.active_adapter in self.lora_scale:
                result += lora_out * self.scaling[self.active_adapter] * self.lora_scale[self.active_adapter]
            else:
                result += lora_out * self.scaling[self.active_adapter]

            # 打印当前 scaling（不考虑 rank，所有进程都会打）
            # if is_rank_0():
            #     try:
            #         val = float(scale.detach().view(-1)[0].cpu())
            #         print("----------------------scaling--------------------------------")
            #         print(f"scale={val:.6f}")
            #         print("----------------------scaling--------------------------------")
            #     except Exception:
            #         pass

            # result += lora_out * scale
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
...
class Embedding(nn.Embedding, LoraLayer):
    ...
    def get_delta_weight(self, adapter):
        base = transpose(self.lora_embedding_B[adapter] @ self.lora_embedding_A[adapter], True)
        if adapter in self.lora_scale:
            return base * self.scaling[adapter] * self.lora_scale[adapter]
        else:
            return base * self.scaling[adapter]
    ...
    def forward(self, x: torch.Tensor):
        if self.disable_adapters:
            ...
            if self.r[self.active_adapter] > 0:
                ...
                if self.active_adapter in self.lora_scale:
                    result += (after_A @ self.lora_embedding_B[self.active_adapter].T) * self.scaling[self.active_adapter] * self.lora_scale[self.active_adapter]
                else:
    ...
    class Linear8bitLt(bnb.nn.Linear8bitLt, LoraLayer):
        ...
        def forward(self, x: torch.Tensor):
            ...
            if self.disable_adapters or self.active_adapter not in self.lora_A.keys():
                ...
                if self.active_adapter in self.lora_scale:
                    scale = self.lora_scale[self.active_adapter]
                else:
