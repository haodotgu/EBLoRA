...
def transpose(weight, fan_in_fan_out: bool):
    """Helper: decide whether to transpose the weight based on fan_in_fan_out"""
    return weight.T if fan_in_fan_out else weight


class LoraLayer:
    def __init__(self, in_features: int, out_features: int, **kwargs):
        # rank and alpha for each adapter (non-trainable)
        self.r = {}
        self.lora_alpha = {}

        # old scaling dictionary can be preserved (compatible with external code), but actual scaling is done with lora_scale parameter
        self.scaling = {}
        ...
        # each LoRA adapter has a trainable scale parameter (scalar)
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            ...
            # 旧的常数 scaling，保留但不再实际使用
            base_scale = lora_alpha / r
            self.scaling[adapter_name] = base_scale

            # 🔹 可学习缩放参数，初始化为 0（即一开始 LoRA 不起作用）
    ...
    def update_layer_embedding(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            ...
            # 🔹 Embedding 路径也给一个可学习缩放参数，初始化为 0
            scale_param = nn.Parameter(
                torch.zeros(
                    1,
                    dtype=self.weight.dtype,
    ...
    def reset_lora_parameters(self, adapter_name):
        if adapter_name in self.lora_A.keys():
            ...
            nn.init.zeros_(self.lora_B[adapter_name].weight)
        if adapter_name in self.lora_embedding_A.keys():
            # initialize A to zero and B to N(0,1)
            nn.init.zeros_(self.lora_embedding_A[adapter_name])
            nn.init.normal_(self.lora_embedding_B[adapter_name])
...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # create LoRA layer and its trainable scale parameter
        self.update_layer(adapter_name, r, lora_alpha, lora_dropout, init_lora_weights)
        self.active_adapter = adapter_name
        self.is_target_conv_1d_layer = is_target_conv_1d_layer
    ...
    def get_delta_weight(self, adapter):
        # LoRA delta W = B @ A * scale
        base = transpose(
            self.lora_B[adapter].weight @ self.lora_A[adapter].weight,
            self.fan_in_fan_out,
        )
        # use trainable scale parameter (scalar), shape is [1], rely on broadcasting to act on the entire matrix
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            result += lora_out * self.lora_scale[self.active_adapter]
